/* 
 * Решение уравнения Фредгольма 2 рода методами:
 * трапеций,
 * Симпсона,
 * 3/8
 * 15 вариант
...
double trapeze(double *s, int idx, double h, double *u)
{
    ...
    for (int i = 0; i < idx; i++)
    {
        sum += 2 * k(s[idx], s[i]) * u[i];
    }
    return (f(s[idx]) + h / 2 * (k(s[idx], s[0]) * u[0] + sum));
}
...
double simpson(double *s, int idx, double h, double *u)
{
    ...
    return (f(s[idx]) + h / 3 * (k(s[idx], s[0]) * u[0] + sum + sum1));
}
...
double three_eights(double *s, int idx, double h, double *u)
{
    ...
    return (f(s[idx]) + 3 * h / 8 * (k(s[idx], s[0]) * u[0] + sum + sum1));
}
...
int main() 
{
	...
	solution(a, b, 10, "Fredgolm2_10.dat");
	solution(a, b, 100, "Fredgolm2_100.dat");
	return 0;
}
